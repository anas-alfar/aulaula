...
<br id=answer_br_' . $i . '  />';
	...
	public function editAction() {
		if ($this -> isPagePostBack) {
			...
			$this -> defaultAnswer = $this -> answerObj -> _totalRecordsFound;
			for ($k = 0; $k < 10; $k++) {
		...
		$answerList = '';
		if (isset($this -> answerObj -> _totalRecordsFound) && !empty($this -> answerObj -> _totalRecordsFound)) {
			$counter = $this -> defaultAnswer = $this -> answerObj -> _totalRecordsFound;
		} else {
			$counter = 10;
			$this -> defaultAnswer = 10;
		}
		for ($i = 1; $i <= $counter; $i++) {
			if (empty($answer[$i]['title']) && empty($answer[$i]['label']) && $i != 1 && $i != 2 && (!isset($this -> answerObj -> _totalRecordsFound) || empty($this -> answerObj -> _totalRecordsFound))) {
				$this -> defaultAnswer--;
				continue;
			}
			$answerList .= '
...
<br id="answer_br_' . $i . '"  />';
	...
	public function listAction() {
		...
		$this -> pagingObj -> _init($this -> pollObj -> _totalRecordsFound);
