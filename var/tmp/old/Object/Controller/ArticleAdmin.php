...
class Object_Controller_ArticleAdmin extends Aula_Controller_Action {
	...
	protected function _init() {
		...
		//source list
		$this -> sourceArticleList = '';
		$this -> sourceArticleListResult = $this -> sourceObj -> getAllObject_sourceOrderById();
		if (!empty($this -> sourceArticleListResult)) {
			foreach ($this->sourceArticleListResult as $key => $value) {
				$selectedItem = ($value['id'] == $this -> view -> sanitized['sourceArticle']['value']) ? 'selected="selected"' : '';
				$this -> sourceArticleList .= '<option value="' . $value['id'] . '"' . $selectedItem . '>' . $value['name'] . '</option>';
			}
		}
		$this -> view -> sourceArticleList = $this -> sourceArticleList;
		//category list
		$this -> categoryList = '';
		...
		$this -> afterIdList = '';
		$this -> afterIdListResult = array();
		//$this->articleObj->getAllObject_articleOrderById();
		if (!empty($this -> afterIdListResult)) {
			foreach ($this->afterIdListResult as $key => $value) {
				...
				$this -> afterIdList .= '<option value="' . $value['id'] . '"' . $selectedItem . '>' . $value['alias'] . '</option>';
			}
		}
		...
		if (!empty($this -> authorsListResult)) {
			foreach ($this->authorsListResult as $key => $value) {
				$this -> authorsList[$value['fullname']] = $value['id'];
			}
			$this -> view -> authorsListWriters = $this -> authorsList;
		}
		if (!empty($this -> authorsListResult)) {
			foreach ($this->authorsListResult as $key => $value) {
				if ($value['fullname'] == $this -> view -> sanitized['originalAuthor']['value']) {
					$selectedItem = 'selected="selected"';
				}
				$this -> authorsList .= '<option value="' . $value['id'] . '"' . $selectedItem . '>' . $value['fullname'] . '</option>';
				$selectedItem = '';
			}
			if (empty($this -> view -> sanitized['originalAuthor']['value'])) {
				$this -> view -> sanitized['originalAuthor']['value'] = -1;
			}

			if (empty($this -> view -> sanitized['originalAuthor']['value']) or $this -> view -> sanitized['originalAuthor']['value'] == -1) {
				if ($this -> view -> sanitized['author_id']['value'] < 1 or empty($this -> view -> sanitized['author_id']['value'])) {
					$this -> view -> sanitized['author_id']['value'] = 0;
					$this -> view -> sanitized['originalAuthor']['value'] = -1;
				}
			}
		...
		$this -> view -> articleTabClass = 'active';
		$this -> view -> articleTabStyle = 'display: block;';
		$this -> view -> photoTabClass = '';
		$this -> view -> photoTabStyle = 'display: none;';
	}
	...
	public function deleteAction() {
		...
		if (!empty($this -> view -> sanitized -> articleId -> value)) {
			foreach ($this->view->sanitized->articleId->value as $id => $value) {
				$articleDelete = $this -> articleObj -> deleteFromObject_articleById($id);
			}
	...
	public function showInObjectAction() {
		...
		if (!empty($this -> view -> sanitized -> articleId -> value)) {
			...
			foreach ($this->view->sanitized->articleId->value as $id => $value) {
				$articleShowInMenu = $this -> articleObj -> updateObject_articleShow_in_objectColumnById($id, $this -> view -> sanitized -> status -> value);
			}
	...
	public function publishAction() {
		...
		if (!empty($this -> view -> sanitized -> articleId -> value)) {
			$this -> view -> sanitized -> status -> value = $this -> view -> sanitized -> status -> value == 'Yes' ? 'Yes' : 'No';
			foreach ($this->view->sanitized->articleId->value as $id => $value) {
				$articlePublish = $this -> articleObj -> updateObject_articlePublishedColumnById($id, $this -> view -> sanitized -> status -> value);
			}
	...
	public function approveAction() {
		...
		if (!empty($this -> view -> sanitized -> articleId -> value)) {
			$this -> view -> sanitized -> status -> value = $this -> view -> sanitized -> status -> value == 'Yes' ? 'Yes' : 'No';
			foreach ($this->view->sanitized->articleId->value as $id => $value) {
				$articleAprrove = $this -> articleObj -> updateObject_articleApprovedColumnById($id, $this -> view -> sanitized -> status -> value);
			}
