...
class Vehicle_Model_Model extends Aula_Model_DbTable {
	...
	public function __construct() {
		$this -> cols = $this -> _cols = array('id', 'vehicle_make_id', 'title', 'description', 'locale_id', 'hash_key', 'date_added', 'comments', 'options');
		$this -> _selectColumnsList = ' SQL_CALC_FOUND_ROWS `id`, `vehicle_make_id`, `title`, `description`, `locale_id`, `hash_key`, `date_added`, `comments`, `options`';
		parent::__construct();
	}
	
	public function getAllModel_OrderByColumnWithLimit( $column, $sorting, $start, $limit ) 
	{
		$start = ( int )($start);
		$limit = ( int )($limit);
		...
		$result = $this 
		-> select() 
		-> from($this->_name. ' as vmo', new Zend_Db_Expr('SQL_CALC_FOUND_ROWS vmo.*'))/* ->    where ('id > ?', 1)*/ 
		-> joinInner('vehicle_make as vma', 'vmo.vehicle_make_id=vma.id',array('vma.title as vehicle_make_title'))
		//-> joinInner('vehicle_type as vt', 'vma.vehicle_type_id=vt.id',array('vt.title as vehicle_type_title', 'vt.id as vehicle_type_id'))
