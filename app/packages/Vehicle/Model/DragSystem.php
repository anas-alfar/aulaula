...
class Vehicle_Model_DragSystem extends Aula_Model_DbTable {

	protected $_name = 'vehicle_drag_system';
	...
	public function getAllDragSystem_OrderByColumnWithLimit( $column, $sorting, $start, $limit ) 
	{
		$start = ( int )($start);
		$limit = ( int )($limit);
	...
	public function getAllDragSystem( ) 
	{
		$result = $this 
		-> select() 
		-> from($this->_name, new Zend_Db_Expr('SQL_CALC_FOUND_ROWS *'))/* ->    where ('id > ?', 1)*/ 
	...
	public function getAllDragSystemByLocalId( $locale_id ) 
	{
	...
	public function getDragSystemById( $id ) 
	{
		$id = (int) $id;
		$result = $this 
		-> select() 
