...
EasyPhpThumbnail class version 2.0.0 - PHP5
...
class Aula_Model_Thumb extends Aula_Model_Default {
	/**
	 *custom variable
	 * where to save the image
	 */
	public $ThumblocationToSave;
	/**
	 *Convert the thumbnail to a polaroid look
	 */
	public $Polaroid;
	/**
	...
	public function __construct() {
		$this->ThumblocationToSave = ''; // custom
		$this->Polaroid = '';
		$this->Thumbsize = 160;
		$this->Thumbheight = 0;
		$this->Thumbwidth = 0;
	...
	private function createemptythumbnail() {
		...
		if ($this->Percentage) {
			if ($thumbwidth > 0) {
				$thumbwidth = floor ( $thumbwidth * ($this->Percentage / 100) * $this->size [0] );
			}
			if ($thumbheight > 0) {
				$thumbheight = floor ( $thumbheight * ($this->Percentage / 100) * $this->size [0] );
			}
	...
	private function maketransparent() {
		...
		imagetruecolortopalette ( $this->thumb, false, 254 );
	...
	private function savethumb() {
		...
		switch ($this->size [2]) {
			case 1 :
				...
				imagegif ( $this->thumb, $this->ThumblocationToSave );
				break;
			case 2 :
				//				imagejpeg($this->thumb,$this->Thumblocation.$this->Thumbprefix.basename($this->image),$this->Quality);
				imagejpeg ( $this->thumb, $this->ThumblocationToSave );
				break;
			case 3 :
				//				imagepng($this->thumb,$this->Thumblocation.$this->Thumbprefix.basename($this->image));
				imagepng ( $this->thumb, $this->ThumblocationToSave );
				break;
		}
		if ($this->Chmodlevel != '') {
			chmod ( $this->Thumblocation . $this->Thumbprefix . $this->image, octdec ( $this->Chmodlevel ) );
		}
		imagedestroy ( $this->im );
		imagedestroy ( $this->thumb );
