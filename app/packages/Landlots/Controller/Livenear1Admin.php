 ...
 * @name Landlots_Controller_Livenear1Admin
...
class Landlots_Controller_Livenear1Admin extends Aula_Controller_Action {
	...
	protected function _init() {
		//default objects
		$this -> livenear1Obj = new Landlots_Model_Livenear1();
		...
		$this -> view -> importExcelLink = '/admin/handle/pkg/landlots-livenear1/action/importcsv/';
		$this -> view -> exportExcelLink = '/admin/handle/pkg/landlots-livenear1/action/exportcsv/';
	}
	...
	public function viewAction() {
		if ( isset($_GET['id']) and is_numeric($_GET['id']) ) {
			...
			$this -> view -> render('landlots/viewLivenear1.phtml');
			exit();
		}
	}

	public function addAction() {
		$form = new Landlots_Form_Livenear1($this -> view);
		...
		if (!empty($_POST) and $form -> isValid($_POST)) {
			...
			$livenear1Data = array('title' => $_POST['mandatory']['title'], 'description' => $_POST['mandatory']['description'], 'locale_id' => $this -> fc -> settings -> locale -> available -> lang -> _1 -> default, 'options' => json_encode($_POST['optional']['options']), 'comments' => $_POST['optional']['comments'], );
			$this -> livenear1Obj -> insert($livenear1Data);
		...
		$this -> view -> form = $form;
		$this -> view -> render('landlots/addLivenear1.phtml');
		exit();
	}

	public function editAction() {
		$form = new Landlots_Form_SimpleLivenear1($this -> view);
		$form -> setView($this -> view);
		...
		$this -> view -> form = $form;
		$this -> view -> render('landlots/updateLivenear1.phtml');
		exit();
	}
	...
	public function deleteAction() {
		...
		if (!empty($this -> view -> sanitized -> livenear1Id -> value)) {
			...
			if (!empty($stmt)) {
				header('Location: /admin/handle/pkg/landlots-livenear1/action/list/success/delete');
				exit();
			}
		}
		header('Location: /admin/handle/pkg/landlots-livenear1/action/list/');
	...
	public function listAction() {
		$this -> view -> arrayToObject($this -> view -> sanitized);
		$this -> view -> sanitized -> actionURI -> value = '/admin/handle/pkg/landlots-livenear1/action/';
		...
		$this -> view -> render('landlots/listLivenear1.phtml');
		exit();
	}
