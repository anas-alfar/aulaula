...
*
* @category Aula
* @package Aula_Estate_Controller_TypeAdmin
...
class Estate_Controller_TypeAdmin extends Aula_Controller_Action {
	...
	protected function _init() {
		//default objects
		$this -> typeObj = new Estate_Model_Type();
		...
		$this -> view -> importExcelLink = '/admin/handle/pkg/estate-type/action/importcsv/';
		$this -> view -> exportExcelLink = '/admin/handle/pkg/estate-type/action/exportcsv/';
	}
	...
	public function viewAction() {
		if ( isset($_GET['id']) and is_numeric($_GET['id']) ) {
			...
			$this -> view -> render('estate/viewType.phtml');
			exit();
		}
	}

	public function addAction() {
		$form = new Estate_Form_Type($this -> view);
		...
		if (!empty($_POST) and $form -> isValid($_POST)) {
			...
			header('Location: /admin/handle/pkg/estate-type/action/list/');
			exit();
		}
		$this -> view -> form = $form;
		$this -> view -> render('estate/addType.phtml');
		exit();
	}

	public function editAction() {
		$form = new Estate_Form_SimpleType($this -> view);
		$form -> setView($this -> view);
		...
		$this -> view -> form = $form;
		$this -> view -> render('estate/updateType.phtml');
		exit();
	}
	...
	public function deleteAction() {
		...
		if (!empty($this -> view -> sanitized -> typeId -> value)) {
			...
			if (!empty($stmt)) {
				header('Location: /admin/handle/pkg/estate-type/action/list/success/delete');
				exit();
			}
		}
		header('Location: /admin/handle/pkg/estate-type/action/list/');
	...
	public function listAction() {
		$this -> view -> arrayToObject($this -> view -> sanitized);
		$this -> view -> sanitized -> actionURI -> value = '/admin/handle/pkg/estate-type/action/';
		...
		$this -> view -> render('estate/listType.phtml');
		exit();
	}
