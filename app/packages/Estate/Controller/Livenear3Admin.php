...
class Estate_Controller_Livenear3Admin extends Aula_Controller_Action {
	...
	protected function _init() {
		//default objects
		$this -> livenear3Obj = new Estate_Model_Livenear3();
		...
		$this -> view -> importExcelLink = '/admin/handle/pkg/estate-livenear3/action/importcsv/';
		$this -> view -> exportExcelLink = '/admin/handle/pkg/estate-livenear3/action/exportcsv/';
	}

	public function addAction() {
		$form = new Estate_Form_Livenear3($this -> view);
		$form -> setView($this -> view);

		if (!empty($_POST) and $form -> isValid($_POST)) {
			$livenear3Data = array('title' => $_POST['mandatory']['title'], 'description' => $_POST['mandatory']['description'], 'locale_id' => $this -> fc -> settings -> locale -> available -> lang -> _1 -> default, 'options' => json_encode($_POST['optional']['options']), 'comments' => $_POST['optional']['comments'], );
			$this -> livenear3Obj -> insert($livenear3Data);

			header('Location: /admin/handle/pkg/estate-livenear3/action/list/');
			exit();
		}
		$this -> view -> form = $form;
		$this -> view -> render('estate/addLivenear3.phtml');
		exit();
	}
	...
	public function editAction() {
		...
		$this -> view -> form = $form;
		$this -> view -> render('estate/updateLivenear3.phtml');
		exit();
	}
	...
	public function deleteAction() {
		...
		if (!empty($this -> view -> sanitized -> livenear3Id -> value)) {
			...
			if (!empty($stmt)) {
				header('Location: /admin/handle/pkg/estate-livenear3/action/list/success/delete');
				exit();
			}
		}
		header('Location: /admin/handle/pkg/estate-livenear3/action/list/');
	...
	public function listAction() {
		$this -> view -> arrayToObject($this -> view -> sanitized);
		$this -> view -> sanitized -> actionURI -> value = '/admin/handle/pkg/estate-livenear3/action/';
		...
		$this -> view -> render('estate/listLivenear3.phtml');
		exit();
	}
