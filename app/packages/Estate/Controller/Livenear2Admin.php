...
class Estate_Controller_Livenear2Admin extends Aula_Controller_Action {
	...
	protected function _init() {
		//default objects
		$this -> livenear2Obj = new Estate_Model_Livenear2();
		...
		$this -> view -> importExcelLink = '/admin/handle/pkg/estate-livenear2/action/importcsv/';
		$this -> view -> exportExcelLink = '/admin/handle/pkg/estate-livenear2/action/exportcsv/';
	}

	public function addAction() {
		$form = new Estate_Form_Livenear2($this -> view);
		$form -> setView($this -> view);

		if (!empty($_POST) and $form -> isValid($_POST)) {
			$livenear2Data = array('title' => $_POST['mandatory']['title'], 'description' => $_POST['mandatory']['description'], 'locale_id' => $this -> fc -> settings -> locale -> available -> lang -> _1 -> default, 'options' => json_encode($_POST['optional']['options']), 'comments' => $_POST['optional']['comments'], );
			$this -> livenear2Obj -> insert($livenear2Data);

			header('Location: /admin/handle/pkg/estate-livenear2/action/list/');
			exit();
		}
		$this -> view -> form = $form;
		$this -> view -> render('estate/addLivenear2.phtml');
		exit();
	}
	...
	public function editAction() {
		...
		$this -> view -> form = $form;
		$this -> view -> render('estate/updateLivenear2.phtml');
		exit();
	}
	...
	public function deleteAction() {
		...
		if (!empty($this -> view -> sanitized -> livenear2Id -> value)) {
			...
			if (!empty($stmt)) {
				header('Location: /admin/handle/pkg/estate-livenear2/action/list/success/delete');
				exit();
			}
		}
		header('Location: /admin/handle/pkg/estate-livenear2/action/list/');
	...
	public function listAction() {
		$this -> view -> arrayToObject($this -> view -> sanitized);
		$this -> view -> sanitized -> actionURI -> value = '/admin/handle/pkg/estate-livenear2/action/';
		...
		$this -> view -> render('estate/listLivenear2.phtml');
		exit();
	}
