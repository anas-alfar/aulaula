...
class Estate_Controller_LocationAdmin extends Aula_Controller_Action {
	...
	public function addAction() {
		$form = new Estate_Form_Location($this -> view);
		$form -> setView($this -> view);

		if (!empty($_POST) and $form -> isValid($_POST)) {
			$locationData = array('title' => $_POST['mandatory']['title'], 'description' => $_POST['mandatory']['description'], 'locale_id' => $this -> fc -> settings -> locale -> available -> lang -> _1 -> default, 'options' => json_encode($_POST['optional']['options']), 'comments' => $_POST['optional']['comments'], );
			$this -> locationObj -> insert($locationData);

			header('Location: /admin/handle/pkg/estate-location/action/list/');
			exit();
		}
		$this -> view -> form = $form;
		$this -> view -> render('estate/addLocation.phtml');
		exit();
	}
	...
	public function editAction() {
		...
		$this -> view -> form = $form;
		$this -> view -> render('estate/updateLocation.phtml');
		exit();
	}
	...
	public function deleteAction() {
		...
		if (!empty($this -> view -> sanitized -> locationId -> value)) {
			...
			if (!empty($stmt)) {
				header('Location: /admin/handle/pkg/estate-location/action/list/success/delete');
				exit();
			}
		}
		header('Location: /admin/handle/pkg/estate-location/action/list/');
	...
	public function listAction() {
		$this -> view -> arrayToObject($this -> view -> sanitized);
		$this -> view -> sanitized -> actionURI -> value = '/admin/handle/pkg/estate-location/action/';
		...
		//$this -> pagingObj -> _init($this -> locationObj -> totalRecordsFound);
		...
		$this -> view -> render('estate/listLocation.phtml');
		exit();
	}
