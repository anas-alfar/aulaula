...
*
* @category Aula
* @package Aula_Estate_Form_Provence
...
class Estate_Form_Provence extends Zend_Dojo_Form
{
	public $view = NULL;
	private $_selectEstateLocationOptions = array();
	...
	private function _getEstateLocationOptionsByLocaleId($locale_id) {
		$estateLocationObj = new Estate_Model_Location();
		$this -> _selectEstateLocationOptions = array (0 => $this -> view -> __('Root'));
		$estateLocationObjResult 	= $estateLocationObj -> select() -> where('locale_id=?',$locale_id) -> query() -> fetchAll();
		if (!empty($estateLocationObjResult)) {
			foreach ($estateLocationObjResult as $key => $value) {
				$this->_selectEstateLocationOptions[$value['id']] = $value['title'];
			}
		}
		return $this->_selectEstateLocationOptions;
	}
    ...
    public function init()
    {
        ...
        $mandatoryForm->addElement(
                ...
                'title',
                array(
                    'label' 	=> $this-> view -> __ ( 'Estate_Title' ),
                    'trim' 		=> true,
                    'required'	=> true,
        ...
        $mandatoryForm->addElement(
                ...
                'description',
                array(
                    'label' 	=> $this-> view -> __ ( 'Estate_Description' ),
                    'trim' 		=> true,
                    'required'	=> true,
                    'class' 	=> 'lablvalue jstalgntop',
                )
        ...
        $mandatoryForm->addElement(
            'FilteringSelect',
            'estate_location_id_'.$value['id'],
            array(
                'label' => $this-> view -> __ ( 'Estate_Location' ),
                'class' => 'lablvalue jstalgntop',
                'autocomplete'=>false,
                'required'	=> true,
                'multiOptions' => $this->_getEstateLocationOptionsByLocaleId($value['id']),
                'id' => 'estate_location_id_'.$value['id'],
				//'onchange' => "dijit.byId('estate_location_id').searchAttr = dijit.byId('estate_type_id').getValue();return true",
            )
        );
		if ($flag === true) {
	        $mandatoryForm->addElement(
        ...
        $optionalForm->addElement(
                ...
                'comments',
                array(
                    'label' 	=> $this-> view -> __ ( 'Estate_Comments' ),
                    'trim' 		=> true,
                    'class' 	=> 'lablvalue jstalgntop',
                	)
            );
        $optionalForm->addElement(
                ...
                'options',
                array(
                    'label' 	=> $this-> view -> __ ( 'Estate_Options' ),
                    'trim' 		=> true,
                    'class' 	=> 'lablvalue jstalgntop',
                	)
            );
