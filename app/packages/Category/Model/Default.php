...
class Category_Model_Default extends Aula_Model_DbTable {
	...
	public function __construct() {
		$this->cols = $this->_cols = array ('id', 'title', 'label', 'description', 'category_type_id', 'user_id', 'parent_id', 'package_id', 'show_in_menu', 'published', 'approved', 'order', 'date_added' );
		$this->_selectColumnsList = ' SQL_CALC_FOUND_ROWS `id`, `title`, `label`, `description`, `category_type_id`, `user_id`, `parent_id`, `package_id`, `show_in_menu`, `published`, `approved`, `order`, `date_added` ';
		parent::__construct ();
	}
	...
	public function getCategoryAndCategory_infoOrderByColumnWithLimit($column = '`id`', $sorting = 'DESC', $start = 0, $limit = 10) {
		...
		$this->_selectQuery = 'SELECT   SQL_CALC_FOUND_ROWS  c.`id`,c.`title`, c.`label`, c.`category_type_id`,c.`parent_id`, 
							c.`package_id`,c.`published`,c.`approved`,c.`order`,c.`date_added`,c.`description`,c.`user_id` , c.`show_in_menu`,
