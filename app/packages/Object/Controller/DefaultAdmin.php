				...
				if ($objectInfoLastInsertId !== false) {
					...
					if ($objectArticleLastInsertId !== false) {
						/*$stmt = $this -> sourceObj -> getAdapter() -> prepare('UPDATE object_source SET `order`=`order`+1 WHERE `order` >= ?');
						 ...
						 $objectSourceData = array('name' => $_POST['source']['name'], 'description' => $_POST['source']['description'], 'source_type' => $_POST['source']['source_type'], 'url' => $_POST['source']['url'], 'order' => $_POST['source']['order'], 'locale_id' => $this->fc->settings->locale->default->current->id, 'author_id' => $_POST['source']['author_id'] = $this -> userId, 'time_delay' => $_POST['source']['time_delay'], 'published' => $_POST['source']['published'], 'approved' => $_POST['source']['approved'], );
